...
export class PermissionManager {
    ...
    grantDocumentRole(docId, userId, role, grantedBy) {
        const now = Date.now();
        const rule = {
            id: uuidv4(),
            docId,
    ...
    getDocumentRole(docId, userId) {
        const docRoles = this.documentRoles.get(docId);
        if (!docRoles)
            return null;
        return docRoles.get(userId)?.role ?? null;
    }
    grantPermission(docId, userId, cellRange, canRead, canEdit, grantedBy) {
        const now = Date.now();
        const rule = {
            id: uuidv4(),
            docId,
    ...
    revokeDocumentRole(docId, userId) {
        const docRoles = this.documentRoles.get(docId);
        if (!docRoles)
            return false;
        return docRoles.delete(userId);
    }
    checkCanEdit(userId, cellId, docId) {
        ...
        const rules = this.getCellRulesForUser(userId, targetDocId);
    ...
    checkCanRead(userId, cellId, docId) {
        const position = this.parseCellId(cellId);
        if (!position)
            return false;
        const targetDocId = docId ?? this.findDocIdByCellId(cellId);
    ...
    getUserEditableRanges(userId, docId) {
    ...
    getUserReadableRanges(userId, docId) {
        const role = this.getDocumentRole(docId, userId);
        if (role === 'owner' || role === 'editor' || role === 'viewer') {
            return [
                {
    ...
    filterCellsByPermission(userId, cells, docId) {
        const allowedCells = [];
        const deniedCells = [];
    ...
    getCellPermissionsForDoc(docId) {
        const result = [];
    ...
    getCellPermissionsForUser(userId, docId) {
        const result = [];
        for (const rule of this.cellPermissions.values()) {
    ...
    getCellRulesForUser(userId, docId) {
        const result = [];
        for (const rule of this.cellPermissions.values()) {
            if (rule.userId !== userId)
                continue;
    ...
    isPositionInRange(position, range) {
        if (range.sheetId !== '*' && position.sheetId !== range.sheetId)
            return false;
