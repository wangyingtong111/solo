...
import { z } from 'zod';
export function createRoutes(documentManager) {
    const router = Router();
    ...
    router.post('/documents', (req, res) => {
        try {
            ...
            const doc = documentManager.createDocument(parsed.name, parsed.ownerId);
    ...
    router.get('/documents/:id', (req, res) => {
        try {
            ...
            const doc = documentManager.getDocumentMeta(docId);
            if (!doc) {
    ...
    router.get('/documents/:id/versions/:versionId', (req, res) => {
        try {
            ...
            const snapshot = documentManager.getVersion(docId, versionId);
            if (!snapshot) {
    ...
    router.get('/documents/:id/diff', (req, res) => {
        try {
            const docId = req.params.id;
            ...
            const diff = documentManager.getDiff(docId, fromVersion, toVersion);
            res.json(diff);
        }
        catch (error) {
            res.status(500).json({ error: error.message });
    ...
    router.get('/documents/:id/permissions', (req, res) => {
        try {
            const docId = req.params.id;
            const rules = documentManager.getPermissionRules(docId);
            res.json(rules);
        }
        catch (error) {
            res.status(500).json({ error: error.message });
    ...
    router.post('/documents/:id/permissions', (req, res) => {
        try {
            const docId = req.params.id;
            const parsed = setPermissionSchema.parse(req.body);
            const permission = documentManager.setPermission(docId, parsed.userId, parsed.role, parsed.grantedBy);
    ...
    router.get('/documents/:id/sheets/:sheetId/cells', (req, res) => {
        try {
            const docId = req.params.id;
            const sheetId = req.params.sheetId;
            const userId = req.query.userId;
            const cells = documentManager.getCells(docId, sheetId, userId);
            res.json(cells);
        }
        catch (error) {
            res.status(500).json({ error: error.message });
