...
import type { DocumentState, VersionSnapshot, VersionDiff } from './types.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

interface StoredVersionSnapshot {
  id: string;
  doc_id: string;
  version: number;
  name: string;
...
export class VersionManager {
  ...
  createSnapshot(docId: string, name: string, createdBy: string): VersionSnapshot {
    const transaction = this.db.transaction(() => {
      ...
      this.db
        .prepare(
          `INSERT INTO version_snapshots (id, doc_id, version, name, cells_json, sheets_json, created_at, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
        )
        .run(
          id,
          ...
          JSON.stringify(state.sheets),
          createdAt,
          createdBy
        );

      const clock: VectorClock = { 'version-manager': 1 };
  ...
  getVersionHistory(docId: string): VersionSnapshot[] {
    const rows = this.db
      .prepare(
        `SELECT id, doc_id, version, name, cells_json, sheets_json, created_at, created_by
         FROM version_snapshots
         WHERE doc_id = ?
         ORDER BY version DESC`
      )
      .all(docId) as StoredVersionSnapshot[];

    return rows.map((row) => this.mapStoredSnapshot(row));
  }

  getVersion(docId: string, versionId: string): VersionSnapshot {
    const row = this.db
      .prepare(
        `SELECT id, doc_id, version, name, cells_json, sheets_json, created_at, created_by
         FROM version_snapshots
      ...
      .get(versionId, docId) as StoredVersionSnapshot | undefined;
  ...
  diffVersions(docId: string, versionAId: string, versionBId: string): VersionDiff {
  ...
  rollbackToVersion(docId: string, targetVersionId: string, userId: string): void {
  ...
  timeTravel(docId: string, timestamp: number): DocumentState {
  ...
  private mapStoredSnapshot(row: StoredVersionSnapshot): VersionSnapshot {
    return {
      id: row.id,
      docId: row.doc_id,
      version: row.version,
  ...
  private reconstructState(docId: string, targetVersion: number): DocumentState {
    ...
    for (const event of events) {
      ...
      switch (event.eventType) {
        case EventType.SHEET_CREATED: {
          const payload = event.payload as SheetCreatedPayload;
        ...
        case EventType.SHEET_UPDATED: {
          const payload = event.payload as SheetUpdatedPayload;
          const sheet = sheets.find((s) => s.id === payload.sheetId);
          if (sheet) {
            if (payload.name !== undefined) sheet.name = payload.name;
            if (payload.index !== undefined) sheet.index = payload.index;
          }
          break;
        }

        case EventType.SHEET_DELETED: {
          const payload = event.payload as SheetDeletedPayload;
        ...
        case EventType.CELL_UPDATED: {
          const payload = event.payload as CellUpdatedPayload;
          const existingCell = cells[payload.cellId];
          const cell: Cell = existingCell
            ? { ...existingCell }
            : {
                id: payload.cellId,
        ...
        case EventType.CELL_FORMULA_UPDATED: {
          const payload = event.payload as CellFormulaUpdatedPayload;
          const existingCell = cells[payload.cellId];
          const cell: Cell = existingCell
            ? { ...existingCell }
            : {
        ...
        case EventType.CELL_STYLE_UPDATED: {
          const payload = event.payload as CellStyleUpdatedPayload;
          const existingCell = cells[payload.cellId];
          const cell: Cell = existingCell
            ? { ...existingCell }
            : {
  ...
  private computeDiff(versionA: VersionSnapshot, versionB: VersionSnapshot): VersionDiff {
  ...
  private computeDiffFromStates(stateA: DocumentState, stateB: DocumentState): VersionDiff {
    const addedCells: Cell[] = [];
    const modifiedCells: Array<{
      cellId: string;
      oldValue: Cell;
      newValue: Cell;
    }> = [];
    ...
    const addedSheets: SheetMeta[] = [];
    const modifiedSheets: Array<{
      sheetId: string;
      oldSheet: SheetMeta;
      newSheet: SheetMeta;
    }> = [];
  ...
  private sheetsEqual(a: SheetMeta, b: SheetMeta): boolean {
    return (
      a.name === b.name &&
      a.index === b.index &&
  ...
  private applyRollbackEvents(
    ...
    const transaction = this.db.transaction(() => {
      const clock: VectorClock = { [userId]: Date.now() };

      for (const sheetId of diff.deletedSheets) {
        this.eventStore.appendEvent(
          docId,
          EventType.SHEET_DELETED,
          { sheetId } as SheetDeletedPayload,
          userId,
          clock
        );
      }
      ...
      for (const mc of diff.modifiedCells) {
        ...
        if (
          JSON.stringify(mc.oldValue.metadata.style) !==
