...
exports.analyzeRuntimeRisks = analyzeRuntimeRisks;
const constants_1 = require("../constants");
function analyzeRuntimeRisks(config, files, options = {}) {
    const risks = [];
...
function checkExposedPorts(config) {
    const risks = [];
    ...
    for (const portSpec of Object.keys(exposedPorts)) {
        ...
        if (constants_1.DANGEROUS_PORTS.includes(portNum)) {
...
function checkVolumes(config, files) {
    const risks = [];
    const volumes = config.Volumes || {};
    for (const volumePath of Object.keys(volumes)) {
        for (const sensitivePath of constants_1.SENSITIVE_PATHS) {
...
function checkHealthcheck(config) {
    const risks = [];
...
function checkSensitiveFiles(files) {
    const risks = [];
    const sshFiles = files.filter((f) => f.path.startsWith('/root/.ssh/') || f.path.startsWith('/etc/ssh/'));
