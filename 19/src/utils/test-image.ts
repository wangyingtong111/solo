...
export async function createTestImage(
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(imageConfigObj, null, 2)
    );
    ...
    const totalPkgs = imageConfig.layers.reduce(
      (sum, l) => sum + (l.packages?.length || 0),
      0
    );
...
export async function createVulnerableTestImage(outputPath: string): Promise<void> {
