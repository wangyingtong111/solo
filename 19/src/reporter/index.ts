...
import { ScanResult, Vulnerability, RuntimeRisk } from '../types';
...
function generateJenkinsReport(result: ScanResult, options: ReportOptions): string {
  const filtered = filterBySeverity(result, options.minSeverity);
  ...
  const jenkinsReport = {
    ...
    issues: jenkinsIssues.map((issue, idx) => ({
      ...
      packageName: (issue as any).packageName || '',
      version: (issue as any).version || '',
      ...
      additionalProperties: {
        cve: (issue as any).cve || '',
        cvss: (issue as any).cvss || 0,
        fixVersion: (issue as any).fixVersion || '',
...
function generateTableReport(result: ScanResult, options: ReportOptions): string {
  const filtered = filterBySeverity(result, options.minSeverity);
  const lines: string[] = [];
  ...
  lines.push('├──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤');
  lines.push(
    `│ ${filtered.summary.critical.toString().padEnd(8)} │ ${filtered.summary.high.toString().padEnd(8)} │ ${filtered.summary.medium.toString().padEnd(8)} │ ${filtered.summary.low.toString().padEnd(8)} │ ${filtered.summary.unknown.toString().padEnd(8)} │ ${filtered.summary.fixable.toString().padEnd(8)} │ ${filtered.summary.total.toString().padEnd(8)} │`
  );
  lines.push('└──────────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┘');
  lines.push('');
  ...
  if (filtered.vulnerabilities.length > 0) {
    ...
    lines.push(
      `${'严重性'.padEnd(10)} ${'CVE ID'.padEnd(18)} ${'软件包'.padEnd(20)} ${'版本'.padEnd(15)} ${'CVSS'.padEnd(6)} 描述`
    );
    lines.push('─'.repeat(100));
    ...
    for (const vuln of filtered.vulnerabilities.slice(0, 50)) {
      ...
      lines.push(
        `${vuln.severity.padEnd(10)} ${vuln.id.padEnd(18)} ${vuln.packageName.padEnd(20)} ${(vuln.packageVersion || '').padEnd(15)} ${(vuln.cvssScore?.toString() || 'N/A').padEnd(6)} ${desc}`
      );
    }
  ...
  if (filtered.runtimeRisks.length > 0) {
    ...
    lines.push('─'.repeat(100));
    lines.push(
      `${'严重性'.padEnd(10)} ${'类别'.padEnd(20)} ${'标题'.padEnd(30)} 描述`
    );
    lines.push('─'.repeat(100));
    ...
    for (const risk of filtered.runtimeRisks.slice(0, 30)) {
      ...
      lines.push(
        `${risk.severity.padEnd(10)} ${risk.category.padEnd(20)} ${risk.title.padEnd(30)} ${desc}`
      );
    }
...
function filterBySeverity(result: ScanResult, minSeverity?: string): ScanResult {
  ...
  if (minRank === -1) return result;

  const filteredVulns = result.vulnerabilities.filter(
    (v) => severityRank.indexOf(v.severity) <= minRank
  );
  const filteredRisks = result.runtimeRisks.filter(
    (r) => severityRank.indexOf(r.severity) <= minRank
  );
