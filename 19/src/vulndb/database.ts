...
export class VulnDatabase {
  ...
  static async create(dbPath: string): Promise<VulnDatabase> {
  ...
  getMeta(key: string): string | null {
    ...
    const result = stmt.getAsObject([key]) as any;
    stmt.free();
  ...
  addVulnerability(vuln: Vulnerability): void {
    ...
    stmt.run([
      ...
      vuln.packageVersion || '',
      vuln.fixedVersion || '',
      vuln.cvssScore !== undefined ? vuln.cvssScore : null as any,
      vuln.cvssVector || '',
  ...
  getAllVulnerabilities(limit = 100, offset = 0): Vulnerability[] {
    if (!this.db) throw new Error('Database not initialized');
...
export function isVersionAffected(
  ...
  if (!affectedRange || affectedRange === '*') return true;
