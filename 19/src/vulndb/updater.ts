...
export class VulnDBUpdater {
  ...
  async updateFromNVD(startDate?: string, endDate?: string): Promise<{ added: number; updated: number }> {
    let added = 0;
    let updated = 0;
    let startIndex = 0;
  ...
  private parseNvdVulns(nvdVulns: any[]): Vulnerability[] {
    ...
    for (const nvdVuln of nvdVulns) {
      ...
      if (cvssMetricV3) {
        ...
        severity = (cvssMetricV3.cvssData?.baseSeverity || 'Unknown') as Severity;
  ...
  private extractAffectedPackages(configs: any[]): { name: string; version?: string }[] {
